%%
%% PT Report Template
%% Representative template for the PT Report class
%%
...
% - Page sides: oneside (default) or twoside
%   oneside: title page 1, content page 2
%   twoside: title page 1, blank reverse page 2, content page 3
% - Shared modules: coreonly/minimal/full, plus the namespaced controls
%   ptlayout/ptnolayout, ptcontent/ptnocontent, and ptruntime/ptnoruntime
% The class is deliberately single-column; twocolumn is not supported.

% Document metadata
\title{Informe Técnico del Proyecto}
\titlesub{Sistema de Gestión Académica}
\titlesubsub{Informe Final}
% \date{2026-08-29}  % Optional; omitted means the current localized date
...
% \workgroup{Grupo de Investigación}
\department{Departamento de Informática}
\school{Escuela de Ingeniería}
\university{Universidad Técnica Federico Santa María}

% Corporate reports can use this hierarchy instead of the institutional one:
% \corporationdepartment{Departamento de Tecnología}
% \corporation{Empresa Ejemplo}

\begin{document}

\maketitle

% Table of contents and conditional lists of registered/captioned entries
\tableofcontents
\ptlistoffigures
\ptlistoftables
\ptlistofcodes
...
Esta es una plantilla representativa que demuestra las capacidades principales
de la clase \inlinecode{pt-report}. El documento está configurado en formato de
una columna con márgenes de 63pt y cada sección comienza en una nueva página.
...
\begin{itemize}
    ...
    \item Portada generada explícitamente mediante \inlinecode{\string\maketitle}
...
\begin{enumerate}
    ...
    \item \textbf{Caras:} oneside (por defecto) o twoside
    \item \textbf{Columnas:} siempre una; twocolumn no está soportado
\end{enumerate}

Con oneside, la portada es la página 1 y el contenido comienza en la página 2.
Con twoside, el reverso de la portada se reserva como página 2 en blanco y el
contenido comienza en la página 3.
...
\begin{table}[htbp]
    \centering
    \begin{tblr}{colspec={|X[l]|C{0.22\linewidth}|C{0.14\linewidth}|}}
        \hline
        \tableheader
...
\begin{table}[htbp]
    \centering
    \begin{tblr}{colspec={|X[l]|C{0.16\linewidth}|C{0.16\linewidth}|C{0.18\linewidth}|}}
        \hline
        \tableheader
        ...
        \hline
    \end{tblr}
    \caption{Resultados cuantitativos del proyecto}
